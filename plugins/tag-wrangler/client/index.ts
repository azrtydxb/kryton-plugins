...
type ActiveOp =
  | { type: 'rename'; tag: string; value: string }
  | { type: 'merge'; tag: string; targetTag: string }
  | { type: 'delete'; tag: string }
  | null;

export function activate(api: ClientPluginAPI): void {
  function TagWranglerPanel(): any {
    ...
    const fetchTags = useCallback(async () => {
      ...
      try {
        ...
        if (resp.ok) {
          const data = await resp.json() as { tags: Tag[] };
          setTags(data.tags);
        } else {
    ...
    function openMenu(tag: string, e: MouseEvent): void {
      e.preventDefault();
      e.stopPropagation();
    ...
    function startMerge(tag: string): void {
      setActionMenu(null);
      // Default to first other tag, or empty
      const other = tags.find((t: Tag) => t.name !== tag);
    ...
    async function doRename(): Promise<void> {
      ...
      try {
        ...
        if (resp.ok) {
          const data = await resp.json() as { updatedNotes: number };
    ...
    function RenameDialog(): any {
      if (!activeOp || activeOp.type !== 'rename') return null;
      return h('div', { className: 'px-3 py-2 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800' },
        ...
        h('div', { className: 'flex gap-1' },
          h('input', {
            ...
            onChange: (e: any) => setActiveOp({ ...activeOp, value: e.target.value }),
            onKeyDown: (e: any) => {
    ...
    // Merge dialog
    function MergeDialog(): any {
      if (!activeOp || activeOp.type !== 'merge') return null;
      const otherTags = tags.filter((t: Tag) => t.name !== activeOp.tag);
      return h('div', { className: 'px-3 py-2 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800' },
        ...
        h('div', { className: 'flex gap-1' },
          h('select', {
            ...
            disabled: busy,
            className: 'flex-1 text-sm px-2 py-1 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100 focus:outline-none focus:border-violet-500',
            onChange: (e: any) => setActiveOp({ ...activeOp, targetTag: e.target.value }),
          },
            otherTags.length === 0
              ? h('option', { value: '' }, '(no other tags)')
              : otherTags.map((t: Tag) => h('option', { key: t.name, value: t.name }, `#${t.name} (${t.count})`))
          ),
          h('button', {
            onClick: doMerge,
    ...
    function DeleteDialog(): any {
      if (!activeOp || activeOp.type !== 'delete') return null;
      return h('div', { className: 'px-3 py-2 border-b border-gray-200 dark:border-gray-700 bg-red-50 dark:bg-red-900/20' },
        h('div', { className: 'text-xs text-gray-700 dark:text-gray-300 mb-2' },
    ...
    return h('div', { className: 'flex flex-col h-full relative' },
        ...
        tags.map((tag: Tag) =>
          h('li', { key: tag.name },
            h('div', {
              ...
              h('span', { className: 'text-sm text-gray-800 dark:text-gray-200 truncate flex-1' },
                h('span', { className: 'text-violet-500 mr-0.5' }, '#'),
                tag.name
              ),
              h('span', { className: 'text-xs text-gray-400 dark:text-gray-500 ml-2 shrink-0' },
                tag.count
              ),
              h('button', {
                onClick: (e: any) => openMenu(tag.name, e),
      ...
      // Footer
      h('div', { className: 'px-3 py-2 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between' },
